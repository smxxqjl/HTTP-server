 ...
 */
void liso_shutdown(char *str, int status)
{
    exit(EXIT_SUCCESS);
}
void signal_handler(int sig)
{
    switch(sig)
    {
        ...
        case SIGTERM:
            ...
            liso_shutdown(NULL, EXIT_SUCCESS);
            break;    
        default:
            break;
            /* unhandled signal */      
...
int daemonize(char* lock_file)
{
    ...
    snprintf(logmessage, 1024, "Successfully daemonized lisod process, pid %d", (int)getpid());
    write(lfp, logmessage, strlen(logmessage));
    return EXIT_SUCCESS;
}
